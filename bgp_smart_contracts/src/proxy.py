...
tx_sender_name = "ACCOUNT" #must add an asn # after account, eg. ACCOUNT151 we do this programmatically later in program
    ...
    if (str(pkt.summary()).find('BGPHeader') > 0) and (pkt[BGPHeader].type == 2) : #dont actually need if filtering on BGP in sniff() function
        ...
        try:
            ...
            #call check on BGPchain to validate segment advertisement request
                    asn=str(pkt[IP].src).split(".")
                    print ("asn: "+asn[1])
                    account_check=tx_sender_name+asn[1]
                    print ("checking validation for ASN: "+account_check)
                    check=bgpchain_validate(adv_segment, account_check)  #previously tx_sender_name
