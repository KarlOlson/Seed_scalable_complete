...
this_results="$results/bench-0-hosts"
[ ! -d "$this_results" ] && {
    mkdir "$this_results"
    collect
}

for ((i=${RESUME:-0}; i<=100; i+=5)); do {
    rm -rf out

    echo "generating emulation..."
    ./lots-of-networks.py --ases 10 --ixs 5 --routers 1 --hosts $i --outdir out --yes --web --ping
    this_results="$results/bench-$i-hosts"
