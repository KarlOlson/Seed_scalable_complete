...
this_results="$results/bench-0-routers"
[ ! -d "$this_results" ] && {
    mkdir "$this_results"
    collect
}

for ((i=${RESUME:-1}; i<=10; i+=1)); do {
    rm -rf out

    echo "generating emulation..."
    ./lots-of-networks.py --ases 10 --ixs 5 --routers $i --hosts 0 --outdir out --yes
    this_results="$results/bench-$i-routers"
