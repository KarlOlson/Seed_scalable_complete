...
### Docker Cleanup:
Sometimes you need to cleanup old containers/networks. The following commands will help:
1. `docker-compose down` stops any running containers. Must run from directory of docker-compose file.
2. `docker image prune -a` removes all inactive containers.
3. `docker network prune` removes inactive networks
